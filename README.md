...
代码里的界面文案一律英文（ComfyUI 审核的要求），中文放在 `locales/zh/main.json`。
中文环境会自动取过来显示中文；这两种界面覆盖节点显示名、输入框旁的文字、节点面板上的按钮，
以及整个 M8 工作台。

判定顺序：ComfyUI 里读设置项 `Comfy.Locale`（读不到退回浏览器语言）；
工作台读 `?lang=` 参数 → `localStorage` → 浏览器语言。
所以想强制中文，浏览器语言设成中文即可，工作台也可以用 `?lang=zh` 显式指定。

**错误提示是中文的，算不算没翻译？**

不算。报错是写给排错的人看的诊断文本，带错误码、按需带详情，
不属于界面标签，所以不做翻译。界面上的按钮、标签、状态行是另一回事，那些已经英文化了。
